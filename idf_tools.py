#!/data/data/com.termux/files/usr/glibc/bin/env python
# coding=utf-8
#
...
class IDFTool(object):
    ...
    def patch_dir(self, patchdir:str, patch_rpath:bool = True):
        #import subprocess
        import patch
        from pathlib import Path
        print(f"Patching {patchdir}")
        #result = subprocess.run(["python", "tools/patch.py", patchdir], capture_output = True, text = True, encoding = "utf-8")
        #if result.returncode != 0:
        #    raise RuntimeError(f"Cannot patch binary: {result.stderr}\n path: {patchdir}")
        patch.patch_directory(Path(patchdir), patch_rpath = patch_rpath)
        print(f"Patched {patchdir}")

    def patch_binary(self, dest_dir: str):
        from pathlib import Path
        # dest_dir like /data/data/com.termux/files/home/.espressif/tools/xtensa-esp-elf-gdb/16.3_20250913
        patch_dir = f"{dest_dir}/{self.name}/bin"
        # 路径不存在就跳过
        if not os.path.exists(patch_dir):
            patch_dir = f"{dest_dir}/qemu/bin"
            if os.path.exists(patch_dir):
                shutil.rmtree(dest_dir)
                raise RuntimeError("抱歉，暂不支持qemu")
            # qemu需要libpixman-glibc,libgcrypt-glibc,pulseaudio-glibc,libxcursor-glibc,libxi-glibc,libxrandr-glibc,libdrm-glibc,libxss-glibc,libwayland*-glibc,libxkbcommon-glibc
            # libSDL2,libslirp,libasound,libsamplerate,libgbm
            print(f"{dest_dir} 不需要修补")
            return
        self.patch_dir(patch_dir)
        # 一些工具，比如 xtensa-esp-elf 需要二次修补
        second_patch_dir = f"{dest_dir}/{self.name}/{self.name}/bin"
        if os.path.exists(second_patch_dir):
            print(f"Tool {self.name} need second patch")
            self.patch_dir(second_patch_dir)
        # /data/data/com.termux/files/home/.espressif/tools/xtensa-esp-elf/esp-14.2.0_20251107/xtensa-esp-elf/libexec/gcc/xtensa-esp-elf/14.2.0/cc1
        if os.path.exists(second_patch_dir):
            print(f"Tool {self.name} need third patch")
            # 这步不能修补rpath，不然会出问题
            dirs = Path(f"{dest_dir}/{self.name}/libexec/gcc/").rglob(f"{self.name}/*")
            for d in dirs:
                self.patch_dir(d, patch_rpath = False)
